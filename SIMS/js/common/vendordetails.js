...
function DataFormationofVendor(result) {
    ...
    for (i = 0; i < jsnData.rows.length; i++) {
        ...
        buttonlnk = '<button id="btnedit" class="btn-action padding-15 btn-edit" type="button" onclick="EditVendorDetails(this,' + jsnData.rows[i].VendorID + ');return false;" rel="tooltip" title="" data-placement="bottom" data-original-title="Edit Vendor Details"></button>';
...
function EditVendorDetails(obj,VendorID)
{
    var dataFromTheRow = $(obj).closest('tr');
    $('#txtName').val(dataFromTheRow.find('td').eq(0).text());
    $('#txtBrandName').val(dataFromTheRow.find('td').eq(1).text());
    $('#txtAddress').val(dataFromTheRow.find('td').eq(2).text());
    $('#txtZip').val(dataFromTheRow.find('td').eq(6).text());
    $("#selCountry option:contains('" + dataFromTheRow.find('td').eq(5).text() + "')").attr("selected", true);
    //$('#selCountry').val(dataFromTheRow.find('td').eq(5).text());
    $('#selCountry').selectpicker('refresh');
    ShowState();
    $("#selState option:contains('" + dataFromTheRow.find('td').eq(4).text() + "')").attr("selected", true);
    //$('#selState').val(dataFromTheRow.find('td').eq(4).text());
    $('#selState').selectpicker('refresh');
    ShowCity();
    $("#selCity option:contains('" + dataFromTheRow.find('td').eq(3).text() + "')").attr("selected", true);
    //$("#selCity").val(dataFromTheRow.find('td').eq(3).text());
    $('#selCity').selectpicker('refresh');
    $('#txtMobile').val(dataFromTheRow.find('td').eq(7).text());
    $('#txtPhone').val(dataFromTheRow.find('td').eq(8).text());
    $('#txtFax').val(dataFromTheRow.find('td').eq(9).text());
    $('#txtEmail').val(dataFromTheRow.find('td').eq(10).text());
